...
- Focus classification runs through `DistractionEvaluator` evidence and the central `ClassificationResolver`: explicit domains outrank explicit apps, explicit apps outrank heuristics, and unknown contexts remain neutral. Mixed-use browser sites like ChatGPT, Gemini, Reddit, and Discord still resolve into a contextual state by default, while generic YouTube/video pages can now surface as distractions unless educational signals or explicit rules say otherwise. FocusEngine consumes one final `ClassificationDecision`; optional local/cloud/visual evidence may only promote a still-neutral context to focus after generation checks, while repeated page-specific corrections can raise confidence for the exact normalized-domain + page-category + intent-category bucket through a privacy-safe learning store without modifying whole-site rules. That learning is profile-aware through the active `FocusIntent`, decays over time, and only surfaces a manual website-rule suggestion after repeated confirmations. The menu bar exposes a safe explanation and immediate app/domain corrections, while optional interaction summaries only adjust ambiguous productive evidence within a bounded cap.
- Launch observability is now explicit: `AppDelegate` records startup branch and launch-time permission state, and `MenuBarController` emits a `menu_bar_status_item_ready` marker once the status item exists so GUI smoke checks can attach to a real readiness signal instead of a fixed delay.
- The menu bar also exposes an explicit "Treat This as Productive" action. For contextual mixed-use sites, the primary correction path is page-scoped first, with website-wide approval only as an explicit secondary choice and app-wide approval last; `MenuBarController` snapshots the current target context when opened so page approvals survive tab changes without over-generalizing into domain-wide rules.
...
- Wave 4 constrains cloud classification to categorical `CloudClassificationInput` values and structured `ClassificationResult` responses; it never sends OCR, screenshots, raw titles, full URLs, browsing history, typed content, or raw interaction data. Social pages now contribute extra title signals like feed/profile/post so the cloud model can validate whether a mixed-use site is actually work-related before promotion. The cloud client speaks OpenAI-compatible chat/responses payloads, LM Studio's native `/api/v1/chat` `input` payload, and Ollama's native chat endpoint so local servers can be used without a protocol mismatch. Visual analysis is an experimental, disabled-by-default final fallback after local text and cloud resolution, running entirely via macOS native Vision APIs (OCR and Image Classification) and completely free of expensive external python/SmolVLM subprocesses.
- The application includes privacy controls to toggle the experimental visual fallback (`PreferencesManager.enableImageClassification`) and the local on-device text check (`enableLocalTextClassification`) during onboarding and in settings; the local text check itself does not depend on an always-on server.
- A bounded in-memory classification cache in `FocusEngine.swift` keys decisions by context identity, active profile ID, active session ID, and classifier-configuration revision. It stores explainable metadata (`source`, `createdAt`) for debugging, and delayed classifier work compares against a shared `RuntimeToken` before applying so stale local/cloud/visual results are discarded before they can mutate state. When any classification-affecting preference, profile change, or the cached cloud API key changes, FocusEngine now bumps that revision, clears in-flight work, and reclassifies the live context on the next main-turn refresh so the visible decision and enforcement state stay aligned with the active settings.
...
- Settings sidebar no longer exposes separate Stats/Hourglass or Analytics/legacy reporting destinations; Analytics is the single reporting surface. A dedicated Productivity Intelligence tab now groups local text, visual fallback, cloud provider, API key, model, and endpoint controls in one place, while Privacy & Data keeps context-history, classification-feedback, interaction-summary, retention/clear, observation count/oldest async, and diagnostics.
...
6. Records launch-time accessibility and screen-recording trust in `DiagnosticsCenter`, then emits `startup_ui_ready` after the accessory menu shell is installed so the installed app can be smoke-tested against a concrete readiness point.
7. History store enforcement lives in storage boundary (`ContextHistoryStore.record` checks isEnabled), not just UI.
...
- `stop()` tears down timers and break state, and clears leftover watch bookkeeping when the engine is shutting down without an active session
...
- keeps mixed-use browser domains contextual by default: `ContextualSiteHeuristic` identifies pages like ChatGPT, Reddit, and Discord; YouTube/video pages remain eligible for page-scoped review, but generic entertainment browsing can still resolve as distracting unless educational signals or explicit rules say otherwise; repeated consistent page corrections can later raise confidence without creating a broad allow rule
...
- keeps a bounded `ClassificationCache` and `inProgressClassifications` set keyed by context/profile/session/config revision so repeated neutral or promoted contexts can be reused without leaking across sessions or preference changes
- uses a shared `RuntimeToken` to reject delayed classifier completions after lifecycle, context, profile, session, or classification-configuration changes, and listens for classification-affecting preference updates plus `KeychainHelper` cloud-key notifications so the current context is re-evaluated instead of waiting for the next app switch
...
- `Anchored/Storage/ContextualLearningStore.swift` (privacy-safe, profile-aware page-scoped correction history for mixed-use domains)
...
- `AnchoredTests/Engine/ClassificationCacheTests.swift` (bounded cache and metadata coverage)
...
- Mixed-use browser domains remain contextual by default; page-scoped approvals may increase confidence for the exact normalized-domain + page-category + intent-category bucket, but they never become broad domain allow rules unless the user explicitly chooses website-wide approval. Repeated confirmations only create a suggestion for a permanent rule; they do not auto-promote one page into a site-wide whitelist.
...
- `FocusEngine` owns significant timer/state logic; now has an injected distraction-countdown timer seam plus `WindowTextExtracting`/`VisualProductivityChecking` and `PreferencesManager`/`ProfileManager`, plus a bounded classification cache and shared runtime token for async classifier invalidation. The contextual-learning path is now profile-aware and recency-weighted, but it is still singleton-heavy via `ProfileManager.activeProfile` and `NSWorkspace.shared` direct calls in some classifiers.
...
- `AppDelegate` is composition root with many singletons; now wired with `FreshInstallChecking`, launch-time permission diagnostics, a startup-ready trace, `ContextHistoryStore.shared`, prefs publishers for history enable/retention, but `DistractionListManager.shared` and `ProfileManager.shared` still global.
...
- `FocusEngine` remains responsible for the session timer, committed-break duration timer, focus prompt timer, declared-activity timer, break-return grace, doomscroll timer, session logging, and overlay delegate calls; the distraction countdown, committed-break duration, focus prompt, break-return grace, doomscroll timer, and active-session expiry now have scheduler seams, but `SessionTimerCoordinator` and `SessionEventRecorder` are still future extractions. The new `RuntimeToken` and bounded `ClassificationCache` reduce stale async work, but the remaining direct `Timer` paths still live here. The `ContextClassifying`/`ClassificationResult` on-device ML seam remains separate from the active visual provider. Cloud key access is still lazy on the first actual cloud request when the cache is cold, so the remaining user-visible prompt risk now sits on the explicit edit/runtime path rather than the settings window opening path.
- The new fog treatment, mission-warning phase, and delayed dim-center reveal are timing-sensitive and should still be smoke-tested in the installed app after overlay changes.
...
- The permission milestone and commitment-lock escape paths are safety-sensitive; test the pre-dim return-to-work flow, the ten-session threshold, quitting while the permission gate is visible, and the launch-time readiness marker that GUI smoke checks should wait for before attaching.
...
- Privacy & Data pane `PrivacySettingsPane` in Settings: history toggle, retention picker, async count/oldest display, clear-all, and `Copy Diagnostic Report` action co-located; cloud/model controls now live in the dedicated Productivity Intelligence tab ✅
...
- FocusEngine: `WindowTextExtracting`/`LiveOCRProvider`, `VisualProductivityChecking`/`LiveVisualProductivityChecker`, `RuntimeToken`, bounded `ClassificationCache`, no `NSClassFromString`, no semaphore, ordered local/cloud/visual fallback pipeline, no OCR in cloud requests, only productive neutral-context promotion, break-return grace that auto-resumes after a stable related work return, and privacy-safe diagnostics events for state transitions, timers, workspace changes, permissions, and sanitized classification outcomes ✅
- Contextual mixed-use site handling: `ContextualSiteHeuristic` keeps ChatGPT/Reddit/Discord-style pages contextual by default, while generic YouTube/video browsing can still resolve as distracting unless educational signals or explicit rules say otherwise; `ContextualLearningStore` persists only coarse privacy-safe normalized-domain + page-category + intent-category learning records, page-scoped corrections can improve the exact bucket without creating a broad domain rule, and repeated confirmations only surface a manual website-rule suggestion ✅
...
- Settings: dedicated Productivity Intelligence tab with SecureField/Keychain persist, model/endpoint fields, provider picker 0 Gemini 1 OpenAI 2 Anthropic 3 Ollama, local visual fallback controls, and explanatory stack copy ✅
...
- `Anchored/Engine/RuntimeToken.swift`
- `Anchored/Engine/ClassificationCache.swift`
...
- `AnchoredTests/Engine/ClassificationCacheTests.swift`
...
- `docs/architecture/anchored-architecture.md`
- `Anchored/MenuBar/SettingsView.swift`
- `Anchored/MenuBar/SettingsSearchSupport.swift`
- `AnchoredTests/App/SettingsSearchSupportTests.swift`
